...
output = apriori_output(freq_rules1) %>% 
  mutate(leverage = interestMeasure(freq_rules1, measure="leverage", trans_class), 
         cosine = interestMeasure(freq_rules1, measure="cosine", trans_class), 
         jaccard = interestMeasure(freq_rules1, measure="jaccard", trans_class),
         rule_power = interestMeasure(freq_rules1, measure="rulePowerFactor", trans_class))
```
...
The below cell shows the rules sorted by lift. In this dataset, the lift for every rule is less than one, meaning that for each rule the antecedent inhibits the consequent. 
```{r}
output %>% arrange(-lift)
```
    
```{r}
#some nodes are items, others are rules
...
* **Cosine** is a measure of correlation between the items in X and Y defined as [(Tan et al. (2004))](https://michael.hahsler.net/research/bib/association_rules/#arules:Tan:2004)
  ...
  * $cosine(A \rightarrow B) = \frac{P(A \cap B)}{\sqrt{P(A)P(B)}} = \sqrt{P(A|B)P(B|A)}$
  * Value between [0, 1]
...
https://www.rdocumentation.org/packages/arules/versions/1.6-6/topics/interestMeasure
