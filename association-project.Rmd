...
Association analysis is an analytical tool used in the examination of datasets, useful for understanding the relationships between elements. Association rule analysis has become an increasingly popular analytical method, because of how critically important it can be for commercial databases. Association analysis can be used any time we have sets of items and a variety of items that may or may not be included in a given set. The goal for this analysis is to uncover relationships between these items. The most practical application for this type of analysis is for retailers, where companies are trying to determine how groups of items are sold together, who would like to find metrics related to these groupings of items. This is often referred to as "market analysis" or "market basket analysis". The value proposition for this type of analysis is massive, as it can help optimize such aspects as recommendation algorithms, special deals, inventory management, and other aspects which are vital to retailers.  
...
Where each cell has a 1 if the transaction has a given item and a 0 otherwise.   
 

Put simply, the goal of this analysis is to find conditions where the probability density of finding a given item in a set $P(x_i)$ is larger than it is outside of said conditions.  

Besides market basket data, association analysis is also applicable to other domains such as bioinformatics, medical diagnosis, and scientific data analysis. In truth, association analysis can be useful whenever one is interested in uncovering interesting or significant relationships within large datasets. For the purposes of this tutorial, we will be walking through a market basket analysis on a sample dataset of grocery purchases.  

We will start by introducing our dataset. Next, we will go more into detail regarding possible applications of this type of analysis. This includes the implementation and uses of the apriori algorithm, a vital algorithm for association rule analysis. Finally we will examine some of the most important interest measures for association rules, including support, confidence, lift, etc. This tutorial is aimed at students with some background in probability theory and statistical computing who are interested in learning more about a common method in the practice of data mining.  
...
To try to visualize our dataset, we plot a simple bar graph. We see that most of our transactions have 9 items in them.
```{r}
...
The author of the arules package, Michael Hahsler, separates support and confidence from the rest of the interest measures based on their importance. We chose only a few alternative interest measures for the sake of this tutorial, but more can be found on [Hahsler’s page](https://michael.hahsler.net/research/association_rules/measures.html).  

## Support and Confidence Review

### Support

* **Support** is a proportion of transactions that contain item A. It can also be thought of as the probability of A being invloved in a future transaction. Low support could indicate that a rule happens by chance.

  * $S(A)$ = (# of transactions containing $A$) / (# total transactions) $= P(A \cap B)$
...
* **Confidence** is an estimate of conditional probability of B being in a basket given the basket already contains A. 
 ...
 * $\text{lift}(A \rightarrow B) = \text{lift}(B \rightarrow A) = \frac{C(A \rightarrow B)}{S(B)} = \frac{C(B \rightarrow A)}{S(A)} = \frac{P(A \cap B)}{P(A)P(B)}$
  ...
  * Lift > 1 indicates positive association between A and B 
    * This means that when A is present in a transaction, B is more likely to be included ($P(B\mid A) > P(B)$).
  * Lift < 1 indicates negative (inhibitive) association between A and B 
    * This means that when A is present in a transaction, B is less likely to be included ($P(B\mid A) < P(B)$).
...
* **Cosine** is a measure of correlation between the items in A and B, as defined in [(Tan et al. (2004))](https://michael.hahsler.net/research/bib/association_rules/#arules:Tan:2004):
  ...
  * A value closer to one indicates a more positive association

  
In this example, all of the cosine measures are relatively low. 
...
Similarly, all of the Jaccard's coefficients are relatively low. 
...
* **Rule Power Factor** weights the confidence of a rule by its support, which can correct for changes in the ratio between the support of a rule and the support of its antecedent (Hasler). Defined as

  * $rpf(A \rightarrow B) = S(A \cup B)  C(A \cup B)$
  
In this example, the rule power factor is also relatively low for all of the rules, meaning a more positive association.
